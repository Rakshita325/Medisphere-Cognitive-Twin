...
public class VitalConsumer {
    ...
    public void consumeVitalData(VitalData vitalData) {
        ...
        try {
            ...
            // Existing MongoDB storage
            vitalRecordRepository.save(record);

            // Part 2: Anomaly Detection
            boolean anomaly = anomalyDetectionService.isAnomaly(vitalData);

            if (anomaly) {

                logger.warn(
                        "ANOMALY DETECTED | Patient: {} | Vital: {} | Value: {}",
                ...
                logger.info(
                        "NORMAL VITAL | Patient: {} | Vital: {} | Value: {}",
                        vitalData.getPatientId(),
                        vitalData.getType(),
                        getDisplayValue(vitalData));
            }

            // Existing FHIR functionality
            createFhirObservation(vitalData);

        } catch (Exception exception) {

            logger.error(
                    "Unable to process vital event for patient {}: {}",
                    vitalData == null
                            ? null
                            : vitalData.getPatientId(),
    ...
    private void validate(VitalData vitalData) {
        ...
        if (vitalData == null
                || vitalData.getPatientId() == null
                || vitalData.getPatientId().isBlank()) {

            throw new IllegalArgumentException(
                    "patientId is required");
        }

        if (vitalData.getType() == null
                || vitalData.getType().isBlank()) {

            throw new IllegalArgumentException(
                    "type is required");
        }

        if (!"Blood Pressure".equalsIgnoreCase(vitalData.getType())
                && vitalData.getValue() == null) {
    ...
    private String getDisplayValue(VitalData vitalData) {

        if ("Blood Pressure".equalsIgnoreCase(vitalData.getType())) {

            return String.valueOf(vitalData.getSystolicBP())
                    + "/"
                    + String.valueOf(vitalData.getDiastolicBP())
                    + " "
                    + vitalData.getUnit();
        }

        return vitalData.getValue()
                + " "
                + vitalData.getUnit();
    }
